...
#include <QDir>
#include <QPainter>
#include <cstdlib>  // Para funciones aleatorias de C++
#include <ctime>    // Para obtener una semilla diferente en cada ejecución
#include <QRandomGenerator>  // Para funciones aleatorias de Qt
...
GameScene::GameScene(QObject *parent)
    ...
    m_deltaX(3), m_deltaY(3), m_height(200)
{
    game::init();
    srand(time(0));
    ...
    // Inicializar y configurar el acuario
    m_acuario.agregarPez(std::make_unique<Pez>("EspecieEjemplo", "AlimentacionEjemplo", 25.0, 7.5, 30.0, 5.0, "HabitatEjemplo", 5.0));

    // Cargar imágenes en el constructor
    //m_fishItem = new QGraphicsPixmapItem(QPixmap(m_listaPeces[0]->getSpecies()));  // Si podria obtener el nombre de la especie como ruta al archivo de imagen
    ...
    m_bgItem = new QGraphicsPixmapItem(QPixmap(m_game.PATH_TO_BACKGROUND_PIXMAP).scaled(game::RESOLUTION.width(), game::RESOLUTION.height()));
    addItem(m_bgItem);
}
